	...
	.page-header.list-header(class=!items.total && !search && !xFilters ? 'empty-list' : '')
		h1
			if items.total
				...
				// span.items-total Nao foi encontrado nennhum conteudo para #{list.plural.toLowerCase()}
				span.items-total Nenhum item encontrado
					= search || xFilters ? ' ' : '.'
	...
	if items.results.length
		...
		.list-pagination
			if items.totalPages > 1
				...
				items.total
				.count Existem #{utils.plural(items.total, '* ' + list.singular, '* ' + list.plural)}
			else 
